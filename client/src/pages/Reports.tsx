    ...
    <div className="space-y-6">
        ...
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center p-6 z-50">
          <div className="bg-white w-full max-w-md rounded-3xl p-8 shadow-2xl">
            ...
            <form onSubmit={handleUpdateUsage} className="space-y-4">
               <div>
                 <label className="text-xs font-black text-slate-400 uppercase">Shop</label>
               ...
               <button className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black shadow-lg hover:bg-indigo-700 transition-all">Save</button>
            </form>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button onClick={handleExport} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-xl font-bold text-xs shadow-sm transition-all active:scale-95">
          ...
          <tbody className="divide-y text-sm">
            {tab === "usage" ? filteredUsages.map(u => (
              <tr key={u.id} className="hover:bg-slate-50 group transition-colors">
                ...
                {currentUser?.role === "admin" && (
                  <td className="px-6 py-4 text-right flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button onClick={() => deleteItem('usage', u.id)} className="p-2 text-slate-400 hover:text-rose-600"><Trash2 size={16} /></button>
                  </td>
                )}
              </tr>
              ...
              <tr key={r.id} className="hover:bg-slate-50 group transition-colors">
                ...
                {currentUser?.role === "admin" && (
                   <td className="px-6 py-4 text-right flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                     <button onClick={() => deleteItem('request', r.id)} className="p-2 text-slate-400 hover:text-rose-600"><Trash2 size={16} /></button>
                   </td>
                )}
              </tr>
            ))}
