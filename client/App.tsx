...
export default function App() {
  ...
  const handleCreateUser = async (
    ...
    try {
      ...
      await db.users.insert({ name, role, password, avatarColor });
  ...
  const handleUpdateUser = async (updatedUser: User) => {
    try {
      ...
      if (currentUser?.id === updatedUser.id) {
        ...
        localStorage.setItem("spareops_user", JSON.stringify(newSession));
      }
    } catch (e) {
      console.error(e);
    }
  ...
  const onHandInventory = useMemo(() => {
    ...
    requests
      ...
      .forEach((r) =>
        r.items.forEach(
          (i) => (counts[i.type] = (counts[i.type] || 0) + i.quantity),
        ),
      );
    usages
          ...
          {view === "dashboard" &&
            (currentUser.role !== "sales" ? (
              <AdminDashboard
                ...
                usages={usages}
                onApprove={(id: string) =>
                  handleUpdateStatus(id, RequestStatus.APPROVED)
                }
                onReject={(id: string) =>
                  handleUpdateStatus(id, RequestStatus.REJECTED)
                }
              ...
              <SalesDashboard
                ...
                onMarkReceived={(id: string) =>
                  handleUpdateStatus(id, RequestStatus.RECEIVED)
                }
            ...
            {pending.length === 0 ? (
              <div className="p-20 text-center">
                ...
                <div className="text-slate-400 font-medium italic">
                  All pending requests cleared from queue.
                </div>
              </div>
            ) : (
              pending.map((r: any) => (
                ...
                >
                  ...
                  <div className="flex gap-3 shrink-0">
                    <button
                      ...
                      className="px-8 py-3 bg-indigo-600 text-white rounded-xl text-xs font-black shadow-xl shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95"
                    >
                      Approve
                    </button>
                    <button
                      onClick={() => onReject(r.id)}
                      className="px-8 py-3 bg-white text-rose-600 border border-rose-100 rounded-xl text-xs font-black hover:bg-rose-50 transition-all active:scale-95"
                    >
                      Reject
                    </button>
                  </div>
        ...
        <div className="bg-white rounded-[2.5rem] border shadow-sm overflow-hidden">
          ...
          <div className="p-10 space-y-6">
            ...
            <button
              disabled={!shop || !part}
              onClick={() => {
                onLogUsage(shop, part, img || undefined);
                setShop("");
                setPart("");
                setImg(null);
              }}
...
function NavItem({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-3 px-3 py-3 rounded-2xl transition-all ${active ? "bg-indigo-600 text-white shadow-xl shadow-indigo-100" : "text-slate-600 hover:bg-slate-100"}`}
    >
      {icon}
  ...
  return (
    <div className="bg-white rounded-[3rem] border shadow-2xl p-12 max-w-2xl mx-auto mt-10 animate-in zoom-in-95 duration-300">
      ...
      <p className="text-center text-slate-400 text-sm font-medium mb-10">
        Select equipment and quantities for replenishment.
      </p>
  ...
  return (
    <div className="bg-white rounded-[3rem] border p-16 shadow-sm space-y-8 animate-in fade-in duration-500">
      ...
      {!aiInsights && !isAnalyzing && (
        <button
          onClick={onAnalyze}
          className="bg-indigo-600 text-white px-12 py-6 rounded-[1.5rem] font-black shadow-2xl shadow-indigo-100 hover:bg-indigo-700 hover:-translate-y-1 transition-all flex items-center gap-3 active:scale-95 group text-lg"
        >
      ...
      {isAnalyzing && (
        <div className="flex flex-col items-center py-16 text-indigo-600 animate-pulse font-black gap-6">
          ...
          <div className="tracking-widest uppercase text-xs font-black">
            Reading consumption patterns...
          </div>
        </div>
      )}
      ...
      {aiInsights && (
        <div className="prose prose-indigo max-w-none bg-slate-50 p-12 rounded-[2.5rem] border border-slate-100 whitespace-pre-wrap font-medium text-slate-800 leading-relaxed shadow-inner animate-in slide-in-from-bottom-4 duration-500 relative">
          <button
            ...
            className="absolute top-8 right-8 p-3 bg-white border rounded-full text-slate-400 hover:text-rose-500 transition-colors shadow-sm"
          >
            <X size={20} />
          </button>
          {aiInsights}
